...
weight_change_dir = "result_weight_change/fedavg11"
output_dir = "result_output_differ/fedavg11"
loss_dir = "result_loss/fedavg11"
acc_dir = "result_acc/fedavg11"
...
class FedAvg11Trainer(BaseTrainer):
    def __init__(self, options, dataset):
        ...
        super(FedAvg11Trainer, self).__init__(options, dataset, worker=worker)
    ...
    def train(self):
        ...
        np.save(loss_dir + '/loss_train_centralized_width4096_tau2_' + self.dataset + self.model, self.loss_list_train_centralized)
        np.save(acc_dir + '/acc_train_centralized_width4096_tau2_' + self.dataset + self.model, self.acc_list_train_centralized)
        np.save(loss_dir + '/loss_test_centralized_width4096_tau2_' + self.dataset + self.model, self.loss_list_test_centralized)
        np.save(acc_dir + '/acc_test_centralized_width4096_tau2_' + self.dataset + self.model, self.acc_list_test_centralized)

        np.save(loss_dir + '/loss_train_width4096_tau2_' + self.dataset + self.model, self.loss_list_train)
        np.save(acc_dir + '/acc_train_width4096_tau2_' + self.dataset + self.model, self.acc_list_train)
        np.save(loss_dir + '/loss_test_width4096_tau2_' + self.dataset + self.model, self.loss_list_test)
        np.save(acc_dir + '/acc_test_width4096_tau2_' + self.dataset + self.model, self.acc_list_test)

        np.save(output_dir + '/output_width4096_tau2_'+ self.model + self.dataset, self.diff_fed_centralized)
        np.save(weight_change_dir + '/weight_change_width4096_tau2_' + self.model+ self.dataset, self.weight_diff_fed_centralized)
