...
def main():
    ...
    length_test = []
    for j in range(10):
        length_train.append(len(mnist_traindata[j]))
        length_test.append(len(mnist_testdata[j]))

    for user in range(NUM_USER):
        for i in range(10):
            num_train = np.round(distribution[i][user]*(length_train[i]-10)).astype(int)
            for s in range(num_train):
                train_X[user].append(mnist_traindata[i][s].tolist())
            mnist_traindata[i] = mnist_traindata[i][num_train:]
